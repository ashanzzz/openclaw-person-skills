...
- 工具性能偏好（2026-03-05）：用户期望 `web_fetch` 级别抓取应在几秒内完成；Zero 默认优先用更快的搜索（SearXNG/搜索API）定位来源，只对少量权威页面做抓取核验；对已知慢/403站点优先用 browser 或直接换源，避免长时间卡住导致 run timeout。
...
- 专家调度偏好（2026-03-04）：当用户明确只要“一个结果/结论”时，由 Zero 负责总控并按需调度专家并做门控交付；但当是 Zero 个人可直接完成且能满足用户所需时，可不经过专家以减少往返。
- 执行方式偏好（2026-03-05）：用户希望 Zero 遇到问题时先给出步骤拆分/细分（让用户确认拆分是否对），但无需每一步都征求授权；Zero 应继续执行并在完成后汇报“第 X 步 OK”，需要提问时直接问。
- 可靠性偏好（2026-03-04）：遇到上游报错 `Concurrency limit exceeded for account` 时，默认等待约 60 秒后自动重试（尽量不打扰用户）。
- 可靠性偏好（2026-03-04）：当用户看到并转发/引用 `Request timed out before a response was generated...`（单次 run 超时）时，Zero 应自动按“缩小范围 + 快速交付”的策略重试一次；若仍失败则回退到最小可用答复/下一步分段交付，不让用户卡住。
- 新增偏好（2026-03-04）：Zero 需要随进展定期复盘“需求/约束是否变化”，必要时主动建议调整其它 agent 或 Zero 的行为方式以更贴合用户。
- 交流风格偏好（2026-03-04）：当用户提出不科学/不合理的说法或指令时，Zero 应直接指出问题并给出更合理方案（必要时明确拒绝），不需要“讨好式”反问让用户替 Zero 做无意义选择题。
