...
version: 2.0.0
description: Export/download ERPNext invoices for a given company + month (e.g., 吉众 2026年1月), including unpaid. Uses ERPNext REST API token from secure/api-fillin.env, saves CSV/JSON/summary under secure/erpnext/exports/. Also serves as the master reference for 阿山's ERPNext operational habits.
---
...
```bash
# 查某公司某月所有采购发票
GET /api/resource/Purchase%20Invoice?filters=[["company","=","天津吉众科技有限公司"],["posting_date","between",["2026-02-01","2026-02-28"]]]
...
## When to use
- 用户说：下载/导出 ERPNext 某公司（如"吉众"）某个月（如 2026-01）的**发票/单据**，**未付款也要**，并要求"记录好、保存好、可复用"。
